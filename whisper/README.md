...
```bash
python whisper/test_asr.py base wavs/sample.wav
```

### 정확도가 중요할 때 (large-v3 모델)

```bash
python whisper/test_asr.py large-v3 wavs/sample.wav
```
...
- **모델 저장 위치**: 모델은 `whisper/models/` 폴더에 저장됩니다 (프로젝트 내 관리).
...
- **모델 자동 다운로드**: 첫 실행 시 모델이 자동으로 `whisper/models/`에 다운로드됩니다.

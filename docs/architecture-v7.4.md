...
> 분산 추적 + 관측성 스택 완성 + audio_gateway 제거
...
│                                                                                 │
│  ┌──────────┐   ┌──────────┐   ┌──────────┐   ┌──────────┐   ┌──────────┐     │
│  │ Frontend │──▶│ Backend  │──▶│  Redis   │──▶│  Worker  │──▶│ LiteLLM  │     │
...
| **redis** | asr-redis | 6379 | 캐시 + Celery Broker | Streams 메시징 |
...
| V7.5 | 예정 | 동적 Health Check 타임아웃, GPU 리소스 오케스트레이션 개선 |

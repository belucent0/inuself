...
class ContentService:
    ...
    async def upload_and_enqueue(
        ...
        logger.info(
            "[Upload] DB에 콘텐츠 생성 완료: content_id=%s, filename=%s",
            content.id,
            file.filename,
        )
        ...
        logger.info(
            "[Upload] 큐에 작업 등록 시도: content_id=%s, filename=%s",
            content.id,
            file.filename,
        )
        ...
        try:
            ...
            logger.info(
                "[Upload] 큐에 작업 등록 성공: content_id=%s, job_id=%s",
                content.id,
                job_id,
            )
        ...
        logger.info(
            "[Upload] 파일 업로드 전체 프로세스 완료: content_id=%s, filename=%s",
            content.id,
            file.filename,
        )
        print(f"[Upload] ========================================")
        print(
            f"[Upload] 파일 업로드 완료: content_id={content.id}, filename={file.filename}"
        )
        print(f"[Upload] ========================================")
        return UploadResponse(content_id=content.id, queued=True)

    async def _upload_file_content_to_storage(
        self, file_content: bytes, object_key: str
    ) -> None:
        """파일 내용을 스토리지에 업로드."""
        ...
        await loop.run_in_executor(
            None, lambda: upload_fileobj(file_obj, key=object_key)
        )
    ...
    async def retry_processing(
        ...
        if file_obj:
            ...
            if retry_type == "ocr":
                ...
                if file_obj.status not in [
                    ...
                    raise ValueError(
                        f"Cannot retry ASR for file with status: {file_obj.status.value}"
                    )
                ...
                if file_obj.status not in [
                    ...
                    raise ValueError(
                        f"Cannot retry LLM summary for file with status: {file_obj.status.value}"
                    )
        ...
        if retry_type == "asr":
            ...
            if hasattr(content, "transcription") and content.transcription:
                # ASR 전사 텍스트
                transcript_data = (
                    content.transcription.get("transcription", {})
                    if isinstance(content.transcription, dict)
                    else {}
                )
            ...
            await self.repo.update_content_status(
                content_id, ContentStatus.SUMMARY_QUEUED
            )

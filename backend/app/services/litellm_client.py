...
async def request_litellm_completion_async(
    *,
    ...
    """
    client = get_async_openai_client(settings.litellm_base_url, settings.litellm_api_key)

    logger.info(
        "[LiteLLM/Async] Request: model=%s",
        settings.litellm_model
    )
        ...
        try:
            ...
            if not stream:
                ...
                if not content:
                    ...
                    reasoning = getattr(response.choices[0].message, 'reasoning', None)
                    if reasoning:
                        logger.info("[LiteLLM/Async] Response: using reasoning (length: %d)", len(reasoning))
                        return reasoning[:200] if len(reasoning) > 200 else reasoning

                    if finish_reason == "length":
                        logger.warning("[LiteLLM/Async] Response: content empty, finish_reason='length'")
                        return "Response generation in progress (token limit)"
            ...
            if exc.status_code == 503:
                ...
                error_message = str(error_body)
                if "Loading" in error_message or "busy" in error_message.lower():
                    logger.info("[LiteLLM/Async] Providers busy or loading, waiting... (%ds)", elapsed)
            ...
            if elapsed < max_retry_time - retry_interval:
                logger.warning("[LiteLLM/Async] Connection error (will retry): %s (%ds)", exc, elapsed)
                await asyncio.sleep(retry_interval)
                elapsed += retry_interval
                continue
            error_msg = f"LiteLLM connection error (retry timeout): {exc}"

...
from .distributed_lock import acquire_lock, _get_redis_client
...
def request_chat_completion(
    *,
    ...
    # 글로벌 락을 사용하여 여러 워커(OCR, LLM 등)가 동시에 서버를 띄우거나 죽이지 않도록 함
    # 최대 2시간(7200초) 대기 (블로킹)
    lock_timeout = 7200.0
    lock_key = "lock:llm_server:global"
    
    logger.info("[LLM] About to check/acquire lock: %s", lock_key)
    
    # 스테일 락 사전 정리: TTL이 없거나 비정상적으로 긴 경우 삭제 후 재시도
    try:
        redis_client = _get_redis_client()
        if redis_client:
            ttl = redis_client.ttl(lock_key)
            if ttl == -1 or (ttl is not None and ttl > lock_timeout * 2):
                logger.warning(
                    "[LLM] Detected stale llama-server lock (%s), ttl=%s, deleting it for recovery",
                    lock_key,
                    ttl,
                )
                redis_client.delete(lock_key)
    except Exception as e:
        logger.warning("[LLM] Failed to check/delete stale llama-server lock: %s", e)
    
    logger.info("[LLM] Attempting to acquire lock with timeout=%s", lock_timeout)
    
    # 락 획득 시도 (짧은 blocking_timeout으로 여러 번 시도)
    # 긴 blocking_timeout(7200초)은 무한 블로킹을 유발할 수 있으므로
    # 30초씩 반복하여 최대 2시간 동안 시도
    max_attempts = 240  # 30초 * 240 = 7200초 (2시간)
    attempt_timeout = 30.0
    
    for attempt in range(1, max_attempts + 1):
        with acquire_lock(lock_key, timeout=lock_timeout, blocking_timeout=attempt_timeout) as acquired:
            if not acquired:
                if attempt < max_attempts:
                    logger.warning(
                        "[LLM] Failed to acquire lock on attempt %d/%d, retrying in 1 second...",
                        attempt, max_attempts
                    )
                    time.sleep(1)
                    continue
                else:
                    raise LlamaServerClientError(
                        f"Failed to acquire global lock for LLM server after {max_attempts} attempts "
                        f"({lock_timeout}s total timeout)"
                    )
            
            logger.info("[LLM] Lock acquired on attempt %d/%d", attempt, max_attempts)
            
            # 락 획득 성공, llama-server 시작 및 API 호출

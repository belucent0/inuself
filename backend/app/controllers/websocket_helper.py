...
async def post_process_with_llm(text: str, base_url: str) -> str:
    """
                ...
                if choices:
                    ...
                    if content:
                        ...
                        try:
                            ...
                            # 그래도 실패하면, 첫 번째 문장만 추출 (설명 제거)
                            # 개행이나 "1.", "**" 같은 마크다운 이전까지만 사용
                            lines = content.split('\n')
                            ...
                            logger.warning(f"[LLM Post-process] All parsing failed, using original text")
                            return text
            
            logger.warning(f"[LLM Post-process] Failed to parse response. Raw content: {content}")
            logger.warning(f"[LLM Post-process] Full JSON Result: {result}")
            logger.warning(f"[LLM Post-process] Failed with status {response.status_code}, using original text")
            return text

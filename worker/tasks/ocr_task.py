...
def process_ocr_task(
    ...
    if file_s3_key:
        logger.info(
            "[Celery OCR] Starting task (Worker preprocessing): file_id={}, task_id={}, file_s3_key={}, ocr_mode={}, ocr_accuracy_mode={}, retry={}",
            file_id, self.request.id, file_s3_key, ocr_mode, ocr_accuracy_mode, self.request.retries
        )
    else:
        logger.info(
            "[Celery OCR] Starting task (Backend preprocessing): file_id={}, task_id={}, images={}, ocr_mode={}, ocr_accuracy_mode={}, retry={}",
            file_id, self.request.id, len(image_s3_keys) if image_s3_keys else 0, ocr_mode, ocr_accuracy_mode, self.request.retries
        )

    # OpenTelemetry trace context 활성화 (분산 추적 연결)
    with trace_celery_task(self, file_id=file_id, pipeline_stage="ocr") as span:
        span.set_attribute("ocr_mode", ocr_mode)
        span.set_attribute("ocr_accuracy_mode", ocr_accuracy_mode)

...
v1.2.0: Worker → ai-gateway (httpx) → ai-asr-vllm · ai-diarize 컨테이너 직결.
...
def run_asr_diarization_pipeline(
    ...
    accuracy_mode: str = "speed",  # compatibility hint; vLLM ASR is the sole backend
...
def _run_case4_parallel_processing(
    ...
    Worker → ai-gateway → ai-asr-vllm/ai-diarize 직접 호출
    - 공유 iGPU 경합을 피하도록 ASR 후 화자분리 실행
    - 두 요청은 동일한 GPU lock을 공유
    """
    print(f"\n{'='*60}")
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=1) as executor:
            # 단일 executor가 ASR 완료 후 diarization을 시작한다.

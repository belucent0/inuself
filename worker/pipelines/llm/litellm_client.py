    ...
    *,
    ...
    while elapsed < max_retry_time:
        try:
            ...
            # 503 "Loading model" 또는 "All providers busy" 에러인 경우 재시도
            if response.status_code == 503:
                try:
                    error_body = response.json()
                    error_message = error_body.get("error", {}).get("message", "")
                    if "Loading" in error_message or "busy" in error_message.lower():
                        logger.info(f"[LiteLLM] Providers busy or loading, waiting... ({elapsed}s)")
            ...
            if elapsed < max_retry_time - retry_interval:
                logger.warning(f"[LiteLLM] Connection error (will retry): {exc} ({elapsed}s)")
                time.sleep(retry_interval)
                elapsed += retry_interval
                continue
